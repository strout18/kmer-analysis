fasta = read.table("xxxxx/promoteristhisright.bed")
colnames(fasta) = c("chr", "start", "stop")
fasta = fasta[,1:3]
fasta$all = paste(fasta$chr, fasta$start, fasta$stop, sep = " ")
max10 = read.table("/xxxxxx/R_FinalFiltered.txt")
...
write.table(ultimate, file = "/xxxxxx/R_PromotersFiltered.txt", quote = FALSE, sep = "\t", row.names = FALSE)
